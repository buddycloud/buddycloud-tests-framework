...
def testFunction(domain_url, session):
	...
	(status, partial_report) = performPostsManagementTests(session, domain_url, api_location, "test_user_channel_not_following", expected_results)

	if status == 0:
		briefing = "Posts management tests for <strong>test_user_channel_not_following@%s</strong> were successful!" % domain_url
	else:
		briefing = "Posts management tests for <strong>test_user_channel_not_following@%s</strong> were not entirely successful!" % domain_url
