...
def testFunction(domain_url, session):
	...
	(status, partial_report) = performMetadataModificationTests(session, domain_url, api_location, "test_user_channel_follower4", expected_results)

	if status == 0:
		briefing = "Metadata modification tests for <strong>test_user_channel_follower4@%s</strong> were successful!" % domain_url
	else:
		briefing = "Metadata modification tests for <strong>test_user_channel_follower4@%s</strong> were not entirely successful!" % domain_url
