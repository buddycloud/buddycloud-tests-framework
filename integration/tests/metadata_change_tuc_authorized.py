...
def testFunction(domain_url, session):
	...
	if status == 0:
		briefing = "Metadata change tests for <strong>test_user_channel_authorized@%s</strong> were successful!" % domain_url
	else:
		briefing = "Metadata change tests for <strong>test_user_channel_authorized@%s</strong> were not entirely successful!" % domain_url
