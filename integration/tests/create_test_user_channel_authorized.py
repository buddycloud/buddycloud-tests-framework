import string
from api_utils import user_channel_exists, create_user_channel, is_authorized_user_channel, close_this_user_channel
from find_api_location import findAPILocation

TEST_USERNAME = "test_user_channel_authorized"
...
def testFunction(domain_url):
	...
	if user_channel_exists(domain_url, api_location, username):
		...
		if ( is_authorized_user_channel(domain_url, api_location, username) 
	...
	if ( len(CLASSIFIED.get('PROBLEM_CREATING', [])) > 0 ):
		...
		status = 1
		briefing = "Could not create some <em>authorized</em> test user channels: "
		briefing += "<strong>%s</strong>" % string.join(CLASSIFIED['PROBLEM_CREATING'], " | ")
		message = "Something weird happened while we tried creating some <em>authorized</em> user channels for testing purposes."
		...
		status = 1
		briefing = "Could not set some user channels privacy setting to <em>authorized</em>: "
		briefing += "<strong>%s</strong>" % string.join(CLASSIFIED['PROBLEM_CLOSING'], " | ")
		message = "Something weird happened while we tried creating some <em>authorized</em> user channels for testing purposes."
		message += "<br/>The following test user channels could not have privacy settings set to <em>authorized</em>: <br/><br/>"
		message += "<strong>%s</strong>" % string.join(CLASSIFIED['PROBLEM_CLOSING'], "<br/>")
		message += "<br/><br/>It seems like your HTTP API is problematic."

	else:

		briefing = "Could successfully assert creation or existance of some <em>authorized</em> user channels: "
		message = "We could assert that <em>authorized</em> user channels which will be used later for testing purposes were either created "
	...
	if ( len(CLASSIFIED.get('EXISTED', [])) > 0 ):
		...
		message += "<br/><br/>The following <em>authorized</em> user channels already existed: <br/><br/>"
	...
	if ( len(CLASSIFIED.get('CREATED', [])) > 0 ):
		...
		message += "<br/><br/>The following <em>authorized</em> user channels were successfully created: <br/><br/>"
